...
// The newer Adafruit TinyUSB library defines its own tud_msc_start_stop_cb.
// We use the library's version and register our callback via setStartStopCallback().
#if 0  // Disabled - conflict with newer TinyUSB library
...
#endif  // #if 0 - disabled tud_msc_start_stop_cb

// Start/stop callback for newer TinyUSB library
static bool msc_start_stop_cb(uint8_t lun, bool start, bool load_eject) {
  (void) lun;
  if (load_eject) {
    if (start) {
      mscStart = true;
